...
<<<<<<< HEAD
# python main.py ${use_case} --wandb_project=$wandbp --seed=$seed --epochs=$eps --optimiser=${default_optimiser} --lr=${default_lr} --batch_size=${default_bs} --random_dim=${default_random_dim}
=======
...
>>>>>>> 4b4f715c3e56a6cfe9a7d37b279055c6845dc0db
...
#   python main_tableGAN.py --wandb_project=$wandbp --seed=$seed --epochs=$eps --eval_epochs_interval=${eval_eps} --use_case=${use_case} --disc_repeats=${rep} --optimiser=${optimiser} --d_lr=${lr} --g_lr=${lr} --pac=${pac} --weight_decay=${weight_decay} --batch_size=${bs}
  done
